...
describe 'test metaprogramacion del motor' do
  ...
  it 'cargo varios suite y veo que se cargaron todos sus test' do
    ...
    expect( motor.lista_de_test_cargados.count ).to eq(8)
  end
...
describe 'test del framework' do
  ...
  it 'pruebo el deberia ser' do
    ...
    expect( lista_resultados.select{ |resultado| resultado.exploto?}.count ).to eq(0)
  end
  ...
  it 'ejecuto todos los test cargados' do
    ...
    expect( lista_resultados.select{ |resultado| resultado.paso?}.count ).to eq(9)
    expect( lista_resultados.select{ |resultado| resultado.fallo?}.count ).to eq(0)
    expect( lista_resultados.select{ |resultado| resultado.exploto?}.count ).to eq(0)
  end
