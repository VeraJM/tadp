...
class DragonBallTest extends FreeSpec with Matchers with BeforeAndAfter {
  ...
  def assertOnCancelada(resultado: Pelea, atc: Guerrero, opo: Guerrero) = resultado match {
    case pelea: PeleaCancelada => pelea.get.shouldBe((atc, opo))
    case _                     => fail
  }
  ...
  "Planes de ataque" - {
    ...
    "Plan de ataque Goku vs Vegeta" in {
      ...
      planDeGoku should be(Some(List(movUsarArmaFilosa, usarItem(SemillaDeErmitanio), movUsarArmaFilosa)))
  ...
  "Peleas hasta el final" - {
    ...
    "Goku la chitea hasta el final con Majin Boo" in {

      val planDeAtaque = List(explotar, movCargarki, movUsarArmaFilosa)
      val estadoFinal: Pelea = goku.pelearContra(majinBuu)(planDeAtaque)
      /* La pelea se cancela porque goku intenta explotar */
     
      assertOnCancelada(estadoFinal, goku, majinBuu)
    }
  }
